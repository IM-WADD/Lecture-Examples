...
function checkNumberOfWords() {
    // Optional check - will only count words when the user types a space to ensure we're only counting complete words
    // A better approach would check for punctuation as well e.g. . or , or !
    if (wordsTextarea.value.charAt(wordsTextarea.value.length - 1) === " ") {
