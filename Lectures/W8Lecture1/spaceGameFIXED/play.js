...
function saveScores() {
    ...
    // Fixes issue 1 - did not check if player is set so player could be "null"
    if (sessionStorage.getItem("spaceGameName") !== null) {
        player = sessionStorage.getItem("spaceGameName");
    }
    if (localStorage.getItem("spaceGameStorage") !== null) {
        // Fixes issue 2 - did not parse the item in storage from JSON to an object
    ...
    // Fixes issue 3 - did not stringify the currentScores object
