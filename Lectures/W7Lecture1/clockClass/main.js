class Clock {
     ...
     */
    constructor(placeName, timeDiff) {
        this.placeName = placeName;
        this.timeDiff = timeDiff;
        this.id = placeName.replace(" ", ""); // removes any spaces
        this.#addNewClock();
    }

    /**
     * Adds the clock to the UI.
     */
    #addNewClock() {
        document.getElementById("clocks").innerHTML += 
            `<div class="column">
                <div id="${this.id}" class="clock">
                    <div class="am-pm" id="${this.id}-am-pm">...</div>
     ...
     */
    #getRotation(value, maxValue)  {
        return value / maxValue * 360;
    }

    /**
     * Display the clock's current time.
     */
    showTime() {
        // Calculates the clock's current time in its timezone
        const time = new Date(); // Gets the date and time in the user's timezone
        const hour = (time.getHours() + this.timeDiff) % 24; // Adds the time difference to the current time then converts to the 0-24 hour range
        ...
        const clockElem = document.getElementById(this.id); // When the clock was created, the place name was used as its id
        // Update the hands
        clockElem.getElementsByClassName("hour")[0].style.transform = "translate(100px) rotate(" + this.#getRotation(convertedHour, 12) + "deg)";
        clockElem.getElementsByClassName("minute")[0].style.transform = "translate(100px) rotate(" + this.#getRotation(minutes, 60) + "deg)";
        clockElem.getElementsByClassName("second")[0].style.transform = "translate(100px) rotate(" + this.#getRotation(seconds, 60) + "deg)";
        // Show AM/PM
        document.getElementById(this.id + "-am-pm").innerHTML = hour < 12 ? "AM" : "PM";
    }
}

const clocks = []; // An array to store clock objects
...
function createClock(event) {
    ...
    const newClock = new Clock(placeName === "" ? "Here" : placeName, Number(document.getElementById("hours-diff").value));
...
// Smoothly scrolls to the add clock form when the page first loads
window.scrollTo({
